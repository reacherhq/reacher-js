...
It is a TypeScript wrapper around the [Reacher Email Verification API](https://reacher.email). Reacher is 100% open-source, written in Rust. It's also free for personal use, and the API token in `@reacherhq/api` is optional, but in this case the requests will be rate-limited.
